...
interface AppViewProps {
  ...
  drops: FoodDrop[]; // Filtered drops for map view
  allDrops: FoodDrop[]; // All drops for donor dashboard
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 pt-40 md:pt-56">
        ...
        {view === 'donor-dashboard' && <DonorDashboard onAddDrop={onAddDrop} myDrops={allDrops.filter(d => d.donorId === user?.email)} />}
        {view === 'sms' && <SMSView drops={allDrops.filter(d => d.status === 'available')} />}
      </main>
